param(
    ...
    [ValidatePattern('^$|^[a-f0-9]{16}$')][string]$Project = '',
    [switch]$NoBrowser
)
...
$launchArguments = @('-X', 'utf8', '-u', (Join-Path $PSScriptRoot 'team_start.py'), '--port', $Port)
if ($Project) { $launchArguments += @('--project', $Project) }
