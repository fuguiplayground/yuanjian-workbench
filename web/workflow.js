...
const AUDIENCE_STEPS=[['research','下拉词采集'],['keywords','关键词清洗'],['audience','人群与场景'],['tower','千机塔洞察'],['strategy','人群策略'],['content','内容与验证']];
...
async function startAI(kind,target='',ids=[],options={}){if(!state.project)throw Error('请先开始一次调研');if(activeAI())throw Error('当前 Codex 任务仍在运行');const p=state.project,projectId=p.id;closeModal();if(kind==='keywords')goto('keywords');if(kind==='insights')goto(options.modules?.length===1&&options.modules[0]==='clean'?'keywords':'insights');const job=await api('/api/projects/'+projectId+'/ai',{revision:p.revision,kind,target,...(ids.length?{ids}:{}),...options});if(state.project?.id!==projectId)return;workflow.aiJob=job;if(kind==='insights')delete workflow.reportChoices[projectId];clearTimeout(workflow.poll);render();await pollAI()}
