...
const INSIGHT_MODULES=[
 {key:'segments',label:'分人群营销策略',description:'按购买动机与人生阶段拆分，逐类输出产品、渠道、内容、达人及促销建议。',source:'all'},
...
// 三步共享原始证据和同一份人群结果，不把项目级选题冒充人群专属策略。
const AUDIENCE_STEPS=[['keywords','整理词库'],['audience','拆分人群'],['strategy','分人群策略']];
function audiencePerson(r){const module=r?.report?.modules?.segments?.status==='success'?'segments':'audience';const rows=r?.report?.modules?.[module]?.status==='success'?r.report.modules[module].report.audiences||[]:[];const key=state.project.id+':'+r?.id;return {rows,key,module,index:Math.min(workflow.audienceSelection?.[key]||0,Math.max(0,rows.length-1))}}
function audienceWorkspace(){
 const page=state.page,r=selectedInsightReport(),{rows,index,module}=audiencePerson(r),person=rows[index];
 const ctx=rows.length?insightModuleContext(r,module):null;
 const point=v=>insightNodesHtml([{type:'point',value:v}],ctx);
 const field=(title,v)=>`<section class="strategy-field"><h3>${esc(title)}</h3>${v?point(v):'<p>旧报告没有这一项。请生成新版人群方案，不以其他结论补齐。</p>'}</section>`;
 const generate=button('生成拆分与策略','generate-segments','','primary',activeAI()?'disabled':'');
 let body='';
 if(!rows.length)body=empty('把词背后的人，和他们为什么买，分清楚','基于当前关键词、原帖和评论生成候选人群与各自的营销方案。只有词时，结果会保留为待验证假设。',generate+button('补充原帖 / 评论','navigate','','','data-page="research"'));
 else if(page==='audience')body=`<p class="source-note">先看差异，再选人群。同一个“送礼”标签，可以是仪式展示、关系表达，也可以是长期纪念；这些只是划分方法示例，不是当前项目结论。</p><div class="audience-candidates">${rows.map((x,i)=>`<article class="report-sheet"><span class="eyebrow">候选 ${i+1} · ${esc(x.priority)}</span><h2>${esc(x.name)}</h2><p>${esc(x.one_line)}</p>${field('发生在什么场景',x.layers?.scene)}${field('为什么买',x.buying_motivation||x.needs?.implicit)}${field('人生阶段 / 当前状态',x.life_stage||x.layers?.social)}<h3>对应原词与证据</h3>${(x.search_terms||[]).map(t=>`<p>${esc(t.text)} ${evidenceButton([t.evidence_id],'查看原词',r.id,module)}</p>`).join('')}${evidenceButton(x.evidence_ids||[],'查看依据',r.id,module)}${button('看这类人的营销策略 →','choose-audience','','primary',`data-index="${i}"`)}</article>`).join('')}</div>`;
 else body=`<div class="audience-context"><label>当前人群 <select id="strategy-audience">${rows.map((x,i)=>`<option value="${i}" ${i===index?'selected':''}>${esc(x.name)}</option>`).join('')}</select></label><p>${esc(person.one_line)}</p></div><section class="report-sheet"><h2>先确定：给谁、解决什么、凭什么</h2><div class="tower-grid">${field('购买动机',person.buying_motivation||person.needs?.implicit)}${field('价值主张',person.positioning?.promise)}${field('产品证明',person.positioning?.proof)}${field('不应承诺什么',person.positioning?.avoid)}</div></section><section class="report-sheet"><h2>这类人的营销方案</h2><p class="caption">方案跟随当前人群切换。所有产品设计、渠道和优惠建议都需实际验证，不代表已有能力或效果。</p>${[['产品策略：特性 / 设计 / 价值','product'],['渠道：在哪里找到这类人','channels'],['内容：痛点 / 场景 / 情感','content'],['达人：谁来讲更可信','creators'],['促销：组合 / 门槛 / 测试','promotion']].map(([title,k])=>field(title,person.marketing?.[k])).join('')}</section><details class="report-details"><summary>展开分析依据与待验证问题</summary>${field('人群优先级依据',person.priority_reason)}${field('当前情绪',person.layers?.emotion)}${(person.barriers||[]).map(x=>field('决策障碍',x)).join('')}<ul>${(person.validation_questions||[]).map(q=>`<li>${esc(q)}</li>`).join('')}</ul>${evidenceButton(person.evidence_ids||[],'查看原始依据',r.id,module)}</details>`;
 return head(page==='audience'?'拆分人群':'分人群策略',page==='audience'?'不是按身份贴标签，而是解释：同一个词下面，为什么有不同的购买理由。':'一次只看一类人，让产品、渠道、内容和促销围绕同一个购买动机。',button('历史报告 / 分析设置','navigate','','small','data-page="insights"'))+flowNav(page)+demoNote()+(activeAI()?aiProgress():'')+(ctx?reportMeta(ctx):'')+(ctx?.report?.limitations?.length?`<p class="source-note">资料缺口：${ctx.report.limitations.map(esc).join('；')}</p>`:'')+body+(rows.length?`<div class="actions">${generate}${button('返回词库补证据','navigate','','','data-page="keywords"')}</div>`:'');
}
...
function flowNav(active){active=({research:'keywords',posts:'keywords',tower:'audience',content:'strategy'}[active]||active);return `<nav class="flow-nav" aria-label="人群策略三步">${AUDIENCE_STEPS.map(([id,label],i)=>`<button class="${id===active?'active':''}" data-action="navigate" data-page="${id}"><span>${i+1}</span>${label}</button>`).join('')}</nav>`}
...
const CLEANING_STEPS=['去重','无效词筛查','5W1H 分类','搜索意图','A1–A5 分层','情绪标注','主题聚合'];
function cleaningWorkspace(r){
 const p=state.project,items=r?.report?.items||[],valid=items.filter(x=>x.valid===true),excluded=items.filter(x=>x.valid===false);
 const classified=keywordClassifications(r),rows=new Map(p.keywords.map(k=>[k.id,k]));
 const trace=x=>{const id=x.id.replace(/^keywords:/,''),k=classified.has(id)?rows.get(id):null;return k?`${k.original_query||'未记录'} / ${k.round??'未记录'}`:'历史快照未记录 / 未记录'};
 const cell=x=>`${esc(reportKeyword(r,x.id))}<div class="secondary">${esc(x.id)}</div>${evidenceButton([x.id.startsWith('keywords:')?x.id:'keywords:'+x.id],'原词依据',r.id,r.module_key||'')}`;
 const themes=[...new Set(valid.map(x=>x.theme).filter(Boolean))];
 return `<section class="report-sheet"><div class="section-head"><div><span class="eyebrow">先清洗，再判断人群</span><h2>下拉词 → 可用关键词库</h2></div>${button(r?'重新清洗关键词':'按七步清洗关键词','generate-clean-library','','primary',activeAI()||!p.keywords.length?'disabled':'')}</div><ol class="cleaning-steps">${CLEANING_STEPS.map(s=>`<li>${s}</li>`).join('')}</ol><details class="report-details"><summary>查看清洗规则与保留边界</summary><p>5W1H 单类优先级：HOW → WHO → WHY → WHEN → WHERE → WHAT。搜索意图与 A1–A5 是词的内容分类，不是购买人数。</p><p>品牌词、竞品词和有意义的陌生词保留；乱码、平台功能词、非本品类词需记录排除原因。售后使用问题移出营销词库，原文保留供产品与服务研究。</p><p>主题随实际品类聚合，不为凑主题数量编造需求。规则排除不等于用户没有需求；抽查后再决定用于哪类研究。</p></details>${r?reportMeta(r):''}${r?`<h3>统计总览 · 本次分析样本</h3><div class="report-numbers">${[['当前入库记录',p.keywords.length],['已分类记录',items.length],['有效记录',valid.length],['排除记录',excluded.length]].map(([l,n])=>`<div><strong>${n}</strong><span>${l}</span></div>`).join('')}</div><p class="caption">以上是记录数，不是搜索量或人数。采集阶段的重复数见第 1 步任务记录；这里不倒推原始总量。${r.scope?.truncated?'本次仅覆盖部分样本，未分析词不计入有效词。':''}</p><h3>清洗后关键词库</h3><div class="table-wrap"><table><thead><tr><th>联想词 / 证据</th><th>原始关键词 / 轮次</th><th>5W1H</th><th>意图 / A1–A5</th><th>情绪 / 主题</th><th>内容建议</th></tr></thead><tbody>${valid.map(x=>`<tr><td>${cell(x)}</td><td>${esc(trace(x))}</td><td>${esc(x.w5h1)}</td><td>${esc(x.intent)} / ${esc(x.stage)}</td><td>${esc(x.emotion||'未标注')} / ${esc(x.theme)}</td><td>${esc(x.content_suggestion||'未形成建议')}</td></tr>`).join('')||'<tr><td colspan="6">本次没有已判定有效的词。</td></tr>'}</tbody></table></div><p class="caption">原始关键词与轮次只展示可核对的当前记录；旧报告未保存的信息不补填。不同来源保留独立证据编号。</p><details class="report-details"><summary>排除词与原因（${excluded.length}）</summary><div class="table-wrap"><table><tr><th>原词</th><th>排除原因</th></tr>${excluded.map(x=>`<tr><td>${cell(x)}</td><td>${esc(x.reason)}</td></tr>`).join('')||'<tr><td colspan="2">本次未标记排除词。</td></tr>'}</table></div></details><h3>主题与人群入口</h3><div class="table-wrap"><table><tr><th>归一主题</th><th>有效记录数</th><th>代表原词</th><th>内容阶段</th></tr>${themes.map(t=>{const group=valid.filter(x=>x.theme===t);return `<tr><td>${esc(t)}</td><td>${group.length}</td><td>${group.slice(0,5).map(x=>esc(reportKeyword(r,x.id))).join('、')}</td><td>${[...new Set(group.map(x=>x.stage).filter(Boolean))].map(esc).join('、')}</td></tr>`}).join('')||'<tr><td colspan="4">待形成有依据的主题。</td></tr>'}</table></div><p class="source-note">主题是人群研究入口，不直接等于人群。下一步结合 WHO、场景词和原文，判断谁遇到了什么问题；排除标记暂未自动限制后续模型取样。</p>`:empty('尚未生成清洗结果','先点击上方入口采集或导入下拉词，再点击「按七步清洗关键词」。原始记录保留，清洗报告单独保存。')}<p class="caption">点击清洗才调用本机 Codex，关键词样本会发送至模型服务；只浏览页面不产生调用。</p></section>`;
}

function keywordWorkspace(){
 const r=latestAI('keywords'),clean=latestKeywordClassification(true);
 return head('整理词库','先整理真实下拉词，再从主题、人群词与场景词进入人群策略。',button('采集 / 导入下拉词','keyword-data','arrow',''))
  +flowNav('keywords')+demoNote()+`<div class="source-note">已有 ${state.project.keywords.length} 条关键词、${(state.project.posts||[]).length} 篇原帖、${state.project.reviews.length} 条评论。下拉词发现线索，原帖和评论用于核对动机。${button('补充原帖与评论','navigate','','small','data-page="research"')}</div>`+jobView()+aiProgress()+cleaningWorkspace(clean)
  ...
  +(r?`<section class="report-sheet">${clean?'<span class="eyebrow">旧版关键词报告</span>':''}<div class="section-head"><h2>${esc(r?.report?.title||'从已有关键词中找需求线索')}</h2>${reportControls('keywords',r)}</div>
  ${r?reportMeta(r)+keywordReportBody(r):empty('关键词采完以后，在这里看结论','点击上方「生成关键词报告」，整理有效词、5W1H、搜索意图、主题与下一步研究方向。')}</section>`:'')
  +`<div class="next-step"><div><h3>继续看：谁在搜索，为什么需要？</h3><p>先把搜索词关联到具体人群与场景，再为每类人制定营销方案。</p></div>${button('查看人群与场景','navigate','arrow','primary','data-page="audience"')}</div>`
}
...
function insightModuleNodes(key,a,r){
 ...
 }else if(key==='audience'||key==='segments'){
  nodes=[group('人群之间的关系',[point(a.audience_map)]),...(a.audiences||[]).map((x,index)=>group(`${index+1}. ${x.name}`,[
   text(x.one_line),...(x.marketing?Object.entries(x.marketing).map(([k,v])=>group(({product:'产品策略',channels:'营销渠道',content:'内容营销',creators:'达人策略',promotion:'促销策略'})[k],[point(v)])):[]),group('优先级 · '+x.priority,[point(x.priority_reason)]),refs(x.evidence_ids),
...
async function startAI(kind,target='',ids=[],options={}){if(!state.project)throw Error('请先开始一次调研');if(activeAI())throw Error('当前 Codex 任务仍在运行');const p=state.project,projectId=p.id;closeModal();if(kind==='keywords')goto('keywords');if(kind==='insights')goto(options.modules?.length===1&&options.modules[0]==='clean'?'keywords':options.modules?.includes('segments')?'audience':'insights');const job=await api('/api/projects/'+projectId+'/ai',{revision:p.revision,kind,target,...(ids.length?{ids}:{}),...options});if(state.project?.id!==projectId)return;workflow.aiJob=job;if(kind==='insights')delete workflow.reportChoices[projectId];clearTimeout(workflow.poll);render();await pollAI()}
...
case'generate-segments':await startAI('insights','',[],{modules:['segments']});return true;
...
case'choose-audience':{const r=selectedInsightReport(),{rows,key}=audiencePerson(r),index=Number(el.dataset.index);if(!Number.isInteger(index)||index<0||index>=rows.length)return true;workflow.audienceSelection??={};workflow.audienceSelection[key]=index;goto('strategy');return true}
case'generate-clean-library':await startAI('insights','',[],{modules:['clean']});return true;
